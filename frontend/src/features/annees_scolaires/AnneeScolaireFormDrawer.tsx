...
interface Props {
  ...
  /** Année à modifier ; absente, le formulaire crée une nouvelle année. */
  annee?: AnneeScolaire | null
  isLoading?: boolean
}

export default function AnneeScolaireFormDrawer({ open, onClose, onSubmit, annee, isLoading }: Props) {
  const [libelle, setLibelle] = useState('')
  const [dateDebut, setDateDebut] = useState('')
  const [dateFin, setDateFin] = useState('')
  const [errors, setErrors] = useState<Errors>({})

  const enEdition = !!annee

  // (Re)pré-remplit le formulaire à chaque ouverture : valeurs de l'année en
  // édition, ou champs vierges pour une création.
  useEffect(() => {
    if (!open) return
  ...
  return (
    <Drawer
      open={open}
      onClose={onClose}
      title={enEdition ? "Modifier l'année scolaire" : 'Nouvelle année scolaire'}
    >
      <form onSubmit={handleSubmit} noValidate className="flex flex-col h-full">
        ...
        <div className="p-4 border-t border-[var(--border)]">
          <Button type="submit" variant="primary" className="w-full" isLoading={isLoading}>
            {enEdition ? 'Enregistrer les modifications' : 'Créer'}
          </Button>
        </div>
      </form>
    </Drawer>
